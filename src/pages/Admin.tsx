...
export default function Admin() {
  ...
  const handleLogout = () => {
    ...
    setStatus({ type: 'success', msg: 'Выход выполнен' });
  };
      ...
      <div className="min-h-screen bg-stone-950 flex items-center justify-center p-4">
        ...
        >
          <div className="flex justify-center mb-8">
            <LayoutDashboard size={60} className="text-brand-orange" />
          </div>
          <h1 className="text-2xl font-black uppercase tracking-tighter mb-4">Admin Access</h1>
          <p className="text-stone-500 text-sm mb-8">Введите пароль для управления сайтом.</p>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button 
              type="submit"
              className="w-full bg-brand-dark text-white py-5 rounded-2xl font-black text-lg hover:bg-brand-orange transition-all shadow-xl shadow-brand-orange/20"
            >
              ВОЙТИ
            </button>
          </form>
          ...
          {status?.type === 'error' && (
            <div className="mt-6 p-4 bg-red-50 text-red-600 rounded-2xl text-sm font-bold flex items-center gap-3">
